...
function JobMatchesPage() {
    ...
    if (saved) {
      try {
        ...
        if (skills && !Array.isArray(skills.sections)) {
          ...
          skills.sections = [{ name: 'Technical Skills', skills: skills.technical || skills.tools || [] }];
  ...
  if (!analysisResult) {
    return (
      <div className="bg-gradient-to-br from-[#0b1326] to-[#060e20] min-h-screen flex flex-col text-[#dae2fd] items-center justify-center">
        ...
        <Link to="/resume-analysis" className="px-6 py-3 bg-[#b4c5ff] text-[#002a78] font-bold rounded-lg hover:brightness-110 active:scale-95 transition-all">
          Upload Resume
        </Link>
      </div>
    );
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'fixed', bottom: '-20%', right: '-10%', width: 700, height: 700, borderRadius: '50%', background: 'radial-gradient(circle, rgba(6,182,212,0.10) 0%, transparent 70%)', pointerEvents: 'none' }} />
      <div style={{ position: 'fixed', top: '40%', right: '20%', width: 400, height: 400, borderRadius: '50%', background: 'radial-gradient(circle, rgba(139,92,246,0.07) 0%, transparent 70%)', pointerEvents: 'none' }} />

      <main style={{ position: 'relative', zIndex: 1 }}>
        <div style={{ maxWidth: 960, margin: '0 auto', padding: '2.5rem 1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
            <div>
              <h1 style={{ margin: 0, fontSize: '2rem', fontWeight: 800, background: 'linear-gradient(135deg,#a5b4fc,#67e8f9)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Your Career Matches</h1>
              <p style={{ margin: '4px 0 0', fontSize: 14, color: '#475569' }}>AI-powered analysis from your resume</p>
            </div>
            <a href="/resume-analysis" style={{
