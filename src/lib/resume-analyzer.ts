...
function calculateSkillMatch(candidateSkills: string[], requiredSkills: string[]): { matched: number; percentage: number; matchedSkills: string[] } {
  ...
  const matchesSkill = (candidate: string, required: string) => {
    ...
    if (normCandidate.split(' ').every(word => normRequired.split(' ').includes(word))) return true;
    if (normRequired.split(' ').every(word => normCandidate.split(' ').includes(word))) return true;
  ...
  return {
    ...
    percentage: requiredSkills.length > 0 ? (matchedSkills.length / requiredSkills.length) * 100 : 0,
    matchedSkills
  };
}
...
function calculateIndustryMatch(candidateExperience: WorkExperience[], jobIndustry: string): number {
  const normalizedJobIndustry = jobIndustry.toLowerCase();
  
  let matchCount = 0;
  candidateExperience.forEach(exp => {
    if (exp.skills.some(skill => normalizedJobIndustry.includes(skill.toLowerCase())) ||
...
function generateCareerInsights(resume: ResumeData): { strongestPath: string; recommendedSkills: string[]; uniqueStrengths: string[] } {
  ...
  if (resume.experience.length > 0) {
    ...
    if (titles.some(t => t.toLowerCase().includes('lead') || t.toLowerCase().includes('senior'))) {
      strongestPath += ' → Leadership Track';
    } else if (titles.some(t => t.toLowerCase().includes('architect') || t.toLowerCase().includes('engineer'))) {
      strongestPath += ' → Technical Track';
    } else if (titles.some(t => t.toLowerCase().includes('analyst') || t.toLowerCase().includes('consultant'))) {
...
export function analyzeResume(resume: ResumeData, jobs: JobOpportunity[]): AnalysisResult {
  ...
  const matches: JobMatch[] = jobs.map(job => {
    ...
    const experienceMatch = calculateExperienceMatch(resume.totalYearsExperience, job.experienceRequired);
    ...
    const totalScore = skillMatch.percentage * 0.4 + experienceMatch * 0.3 + educationMatch * 0.15 + industryMatch * 0.15;
    ...
    if (resume.totalYearsExperience < job.experienceRequired) {
      preparationSteps.push(`Gain ${job.experienceRequired - resume.totalYearsExperience} more years of relevant experience`);
    }
    preparationSteps.push('Review the job description and prepare examples from your experience');
    
    if (preparationSteps.length === 0) {
      preparationSteps.push('You\'re well-prepared! Focus on polishing your portfolio and interview skills');
    }
